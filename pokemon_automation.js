...
async function generateListing(prompt) {
  ...
  var res = await fetch(
    "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-image-preview:generateContent?key=" + NB_API_KEY,
    {
      ...
      body: JSON.stringify({
        contents: [{ parts: [{ text: "Based on this Pokemon art description: \"" + prompt + "\"\n\nGenerate an optimized Etsy product listing. Respond with raw JSON only, no markdown, no backticks:\n{\n  \"title\": \"Etsy optimized title under 80 chars. Format: Pokemon [Scene Description] Canvas Print [Theme] Wall Decor. Use the word Pokemon but DO NOT use any specific character names like Pikachu Charizard Eevee etc. Examples: Pokemon Sunset Canvas Print Kawaii Wall Decor. Pokemon Winter Forest Canvas Print Anime Wall Decor. Pokemon Night Sky Canvas Print Kawaii Wall Decor. NO character names, NO dashes, NO special characters.\",\n  \"description\": \"3 engaging paragraphs about this specific artwork scene, the canvas print quality, and who would love it as a gift. Do not mention specific character names.\",\n  \"tags\": [\"IMPORTANT: exactly 13 tags, each tag must be under 20 characters, no special characters. Use only generic terms: kawaii wall art, anime wall art, kids room art, nursery art, kawaii canvas, cute wall art, anime poster, kawaii decor, fantasy art, cute kids decor, kawaii gift, anime art gift, cute room decor\"]\n}" }] }],
