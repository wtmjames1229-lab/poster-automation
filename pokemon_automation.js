...
async function generateListing(prompt) {
  ...
  var res = await fetch(
    "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-image-preview:generateContent?key=" + NB_API_KEY,
    {
      ...
      body: JSON.stringify({
        contents: [{ parts: [{ text: "Based on this art description: \"" + prompt + "\"\n\nGenerate an optimized Etsy product listing. Respond with raw JSON only, no markdown, no backticks:\n{\n  \"title\": \"Etsy optimized title under 80 chars. Describe the scene and art style using only generic terms. NO character names, NO franchise names, NO brand names. Use words like: Kawaii, Anime, Retro, Cute, Creature, Fantasy, Vintage, Canvas Print, Wall Decor, Art Print, Kids Room, Nursery. Examples: Kawaii Creature Retro Canvas Print Anime Wall Decor. Cute Fantasy Creature Canvas Print Retro Wall Art. Retro Anime Art Canvas Print Kawaii Home Decor. NO dashes, NO special characters.\",\n  \"description\": \"3 engaging paragraphs about this specific retro poster artwork, the bold eye-catching design, the canvas print quality, and who would love it as a gift. Do not mention any character or brand names.\",\n  \"tags\": [\"IMPORTANT: exactly 13 tags each under 20 characters no special characters. Examples: kawaii wall art, anime wall art, kids room art, nursery art, retro anime art, cute wall art, anime poster, kawaii decor, vintage anime art, retro wall decor, kawaii gift, anime art gift, cute room decor\"]\n}" }] }],
